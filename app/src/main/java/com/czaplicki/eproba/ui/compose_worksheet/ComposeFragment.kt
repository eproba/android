...
class ComposeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        if (EprobaApplication.instance.apiHelper.user!!.function < 2) {
            binding.userSelect.editText?.setText(user.nickname)
        }

        val initialData: Worksheet? = Gson().fromJson(
            arguments?.getString("initialData"),
            Worksheet::class.java
        )
        ...
        if (initialData != null) {
            binding.worksheetName.editText?.setText(initialData.name)
            if (EprobaApplication.instance.apiHelper.user!!.function >= 2) {
        ...
        binding.clearButton.setOnClickListener {
            ...
            binding.worksheetName.editText?.setText("")
        ...
        binding.submitButton.setOnClickListener {
            ...
            val userId: UUID? =
                if (user.function >= 2 && binding.userSelect.editText?.text.toString()
                        .isNotBlank()
                ) {
            ...
            if (name.isBlank() || userId == null || tasks.isEmpty()) {
                ...
                lifecycleScope.launch {
                    try {
                        EprobaApplication.instance.database.worksheetDao().insert(
                            EprobaApplication.instance.service.createWorksheet(
                                Worksheet(
                                    name = name,
                                    userId = userId,
                        ...
                        activity?.runOnUiThread {
                            MaterialAlertDialogBuilder(
                                ...
                                .setTitle(com.czaplicki.eproba.R.string.worksheet_created)
                        ...
                        if (e is HttpException && e.code() == HTTP_FORBIDDEN) {
                            activity?.runOnUiThread {
                                MaterialAlertDialogBuilder(
                                    ...
                                    .setMessage(com.czaplicki.eproba.R.string.worksheet_creating_unauthorized_error)
                            ...
                            activity?.runOnUiThread {
                                MaterialAlertDialogBuilder(
                                    ...
                                    .setMessage(
                                        getString(
                                            com.czaplicki.eproba.R.string.worksheet_creating_error,
                                            e
                                        )
                                    )
                                    .setIcon(com.czaplicki.eproba.R.drawable.ic_error)
